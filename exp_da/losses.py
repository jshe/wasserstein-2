...
def calc_optimal_ineq(x, y, tx, ty, phi, psi, cost, lambda_ineq, loss):
    ux, vtx = phi(x), psi(tx)
    uty, vy = phi(ty), psi(y)
    return loss(x, tx, ux, vtx, cost, lambda_ineq) + ineq_loss(ty, y, uty, vy, cost, lambda_ineq)
...
def gp_loss(x, y, d, lm, clamp=True):
    batch_size = x.size()[0]
