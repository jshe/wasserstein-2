...
def main():
    ...
    if config.solver != 'none':
        X = model.g(X)
    X, y = utils.to_data(X), utils.to_data(y)
    classifier.fit(X.reshape(X.shape[0], -1), y.reshape(-1))

    Xhat, yhat = get_data(config, train=False)
    Xhat, yhat = utils.to_data(Xhat), utils.to_data(yhat)
    pred = classifier.predict(Xhat.reshape(Xhat.shape[0], -1))
    accurracy = (pred == yhat.reshape(-1)).astype(float).mean()
    print('Classification accuracy: %06d' % (accuracy))
